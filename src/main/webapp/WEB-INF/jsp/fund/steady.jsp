        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
			<div class="panel panel-default">
			  <div class="panel-heading">
				<h3 class="panel-title"><i class="glyphicon glyphicon-th"></i> 数据列表 -- 稳健型基金配置</h3>
			  </div>
...
<button type="button" class="btn btn-primary" style="float:right;" onclick="addFund('steady')"><i class="glyphicon glyphicon-plus"></i> 新增</button>
<br>
 <hr style="clear:both;">
	...
	<script src="${APP_PATH}/layer/layer.js"></script>
        <script type="text/javascript">
            	...
            	$.ajax({
            		...
            		success : function(result) {
            			...
            			if ( result.success ) {
            				...
            				var fundSteadyPage = result.data;
            				var fundSteadys = fundSteadyPage.records;
            				
            				$.each(fundSteadys, function(i, fundSteady){
            	                tableContent += '<tr>';
	          	                tableContent += '  <td>'+(i+1)+'</td>';
	          					tableContent += '  <td><input type="checkbox" name="fundid" value="'+fundSteady.id+'"></td>';
	          	                tableContent += '  <td>'+fundSteady.fundCode+'</td>';
	          	                tableContent += '  <td>'+fundSteady.fundName+'</td>';
	          	                tableContent += '  <td>'+fundSteady.fundType+'</td>';
								tableContent += '  <td>'+fundSteady.netWorth+'</td>';
								tableContent += '  <td>'+fundSteady.expectGrowth+'</td>';
								tableContent += '  <td>'+fundSteady.dayGrowth+'</td>';
								tableContent += '  <td>'+fundSteady.monthGrowth+'</td>';
								tableContent += '  <td>'+fundSteady.threeMonthsGrowth+'</td>';
								tableContent += '  <td>'+fundSteady.sixMonthsGrowth+'</td>';
								tableContent += '  <td>'+fundSteady.yearGrowth+'</td>';
								tableContent += '  <td>'+fundSteady.manager+'</td>';
								tableContent += '  <td>'+fundSteady.investment+'</td>';
								tableContent += '  <td>'+fundSteady.investmentRation+'</td>';
	          	                tableContent += '  <td>';
								tableContent += '  <button type="button" onclick="updateMoney('+fundSteady.id+')" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';
	          					tableContent += '  <button type="button" onclick="deleteFund('+fundSteady.id+')" class="btn btn-danger btn-xs"><i class=" glyphicon glyphicon-remove"></i></button>';
	          					tableContent += '  </td>';
	          	                tableContent += '</tr>';
            				});
            				...
            				for ( var i = 1; i <= fundSteadyPage.pages; i++ ) {
            					if ( i == pageNum ) {
            				...
            				if ( pageNum < fundSteadyPage.pages ) {
			...
			function updateMoney(id) {
				window.location.href = "${APP_PATH}/fund/edit?fundId="+id;
			}
        </script>
  </body>
</html>
